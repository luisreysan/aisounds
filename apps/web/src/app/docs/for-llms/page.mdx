...
export const metadata = {
  title: 'For AI agents · AI Sounds Docs',
  description: 'How AI coding assistants can install and configure aisounds packs on behalf of a user.',
}

<div className="not-prose mb-6 flex justify-end">
  <CopyPageButton source="/docs-md/for-llms.md" />
</div>
...
There is also a machine-friendly index at [`/llms.txt`](/llms.txt) and a self-contained dump at [`/llms-full.txt`](/llms-full.txt).
...
When in doubt, fetch [`/llms.txt`](/llms.txt) and follow the links from there. Or paste the user's question into the search and follow the relevant doc page.
